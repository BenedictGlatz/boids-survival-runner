...
- The HUD says which kind of boid the current wave is sending in: `SPAWNING`, at the top centre, a level from 01 to 05 with a small dart beside it, both in the colour that variant is actually drawn in. The escalation existed from the start — every wave sends a faster, wider-seeing, more crowding variant than the last, up to wave five — but nothing ever said so, so the only way to notice was that the swarm had quietly become harder. The colour is the real announcement here rather than the number: it is the same colour the darts out there wear, so the value needs no legend and points at the boids it describes. It also shows where the ramp stops. From wave five on the level stays at 05 while the wave counter beside it keeps climbing, which is exactly the truth of the late game — from there the swarm only grows in numbers, not in kind.
...
- The wave number moved out of the top-right corner and now stands at the top centre, in one group with the round timer and the new spawning level, separated by hairlines. The three answer one question between them — how far am I, and what is coming now — and answering it used to take two looks at opposite corners of the screen. It is now one glance upwards, and the corners are left to the score and the boid count, which are the two values you check between waves rather than during one. The timer stays the largest of the three, because it is the one that changes every second.

- The boid count at the bottom right is finally red, the way the rest of the interface has been saying it should be: red is the colour of everything that costs you something, and the number of things hunting you is the plainest example of that. It had been rendering in plain white because the rule that colours it was keyed to a class the element never carried.

- The player turns far more sharply. Changing direction used to be an almost entirely passive affair: while a movement key was held nothing braked the velocity you already had, so the only force working against the old direction was the acceleration towards the new one. A full reversal therefore took 0.6 s, and a ninety-degree turn was worse than it looked — the sideways part of the velocity was never actually braked at all, it only bled away as the speed cap redistributed it, which took **1.6 s** to clear. That is the sliding, weighty feeling the controls had, and it was neither intended nor a matter of the numbers being too low. The player now brakes the part of their velocity that is not going where they are pointing, at its own and higher rate: a ninety-degree turn clears in 0.1 s and a reversal takes 0.28 s. Getting going and stopping are quicker too — 0.18 s to top speed instead of 0.30, and 0.15 s to a standstill instead of 0.25. The top speed itself is unchanged, and so is the dash: neither is faster than it was, they are only easier to point. The one deliberate consequence is that a dash can now be steered out of, since the same brake acts on its surplus — dash and hold the same direction and it goes exactly as far as it always did.

- A power-up marker that an obstacle grows over now leaves the arena instead of sitting inside the hazard. Markers have always been placed clear of every obstacle, and still are — but obstacles keep arriving throughout a round, roughly as often as markers do and each standing for forty seconds, so sooner or later one materialises on top of a marker that was already lying there. The marker was then bait: visibly there, and inside the one thing in the arena that pushes you out of it and takes a life for the trouble. It now scales away over a third of a second rather than blinking out, because a marker that disappears in front of you reads as theft while one that visibly leaves explains itself, and it does so during the second and a half a new obstacle spends materialising — so it is gone before the hazard that took it can cost anyone anything. It also cannot be picked up on the way out. A marker an obstacle merely appeared _near_ is left exactly where it is: the distance that ends a marker's stay is far tighter than the one it is placed with, or the cure would take more markers than the problem does.
