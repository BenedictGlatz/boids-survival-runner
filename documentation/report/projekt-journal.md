...
| 2026-08-20 | 1,0 | D-01 | Kapitel 09 auf das Seitenbudget von ~1 S. gekürzt und entlang der Muster-Vorlage neu geschnitten: Vorspann, 9.1 Größe und Verteilung mit Tabelle 8, 9.2 Coverage, 9.3 Weitere Masszahlen. Von 245 auf 117 Zeilen (rund 1.000 Wörter) gebracht, ohne eine Aussage zu verlieren — die drei ausgelagerten Teile stehen jetzt im Anhang und werden von dort referenziert: der Befehlsblock als _Erhebungsbefehle der Masszahlen_ unter 11.3 (der erste konkrete Eintrag in diesem bis dahin leeren Abschnitt), die Commit-Typ-Tabelle als _Commits nach Conventional-Commit-Typ_ unter 11.1, und die zweizeilige Coverage-Tabelle ersatzlos, weil die Langfassungen je Modul dort ohnehin liegen. Damit trägt das Kapitel wie im Muster genau **eine** nummerierte Tabelle. Die Abschnittsnummerierung ist bei der Gelegenheit vom Artefakt `9.2b` befreit (9.2 → 9.1, 9.2b → 9.2, 9.3 bleibt); die fünf externen Querverweise in Kap. 04, 05 und im Anhang sind mitgezogen, der historische Journaleintrag vom 29.07. bewusst nicht. Zwei Sachfehler beim Umschreiben gefunden und behoben: der Vorspann nannte „drei Ebenen" und zählte vier auf, und die Locale-Schlüssel standen als eigener Absatz in 9.3, obwohl sie in den Sprachen-Satz des Vorspanns gehören |
| 2026-08-16 | 1,5 | D-01 | Abgabefassung `documentation/projekt-und-architekturdokumentation.md` aus den Kapiteln 01–10 zusammengeschrieben — die redaktionell durchgesehene Fassung für den Word-Übertrag, im Unterschied zur mechanischen Zusammenkopie `gesamtdokument.md`. Enthalten sind Kapitel 1–10 in voller Länge plus Vorspann-Platzhalter (Titelblatt, Verzeichnisse) und ein Abkürzungsverzeichnis; nicht enthalten sind Arbeitsindex, Anhang, KI-Verzeichnis und Journal, die separat übergeben werden. Redaktionelle Eingriffe: die Arbeits-Metadaten der Kapitelköpfe (Seitenbudget/Status/Quellen) und die Arbeitsnotizen der Vorreden von Kap. 7 und 8 entfernt bzw. zu Leser-Prosa umgeschrieben; `9.2b Coverage` zu `9.3` regularisiert und `9.3 Weitere Masszahlen` zu `9.4`, samt Nachziehen aller Querverweise in Kap. 4, 5 und 10; die vier Mermaid-Diagramme (Eingabekette, Dash-Cluster, Sprachgrenze, Frame-Sequenz) als nummerierte Abbildungen 1–4 mit Bildunterschrift gesetzt — auf Nachfrage als Mermaid-Quellen statt der zunächst gesetzten Text-Platzhalter, extrahierbar für Word über das geplante `docs:diagrams`; Dateinamen im Fließtext durchgängig kursiv statt als Code gesetzt, der Muster-Konvention folgend. Vor dem Schreiben fünf zentrale Konstanten gegen den Code geprüft (`PLAYER_DASH_SPEED` 1100, `PLAYER_MAX_SPEED` 360, `INITIAL_BOID_COUNT` 24, `DASH_SELECTION_INTERVAL_STEPS` 24, die drei Strides 7/3/5) — alle Angaben der Kapitel stimmen mit dem Stand des Codes überein |
| 2026-08-16 | 1,5 | D-01 | Abgabefassung `documentation/projekt-und-architekturdokumentation.md` von rund 26.000 auf rund 8.300 Wörter gekürzt (Vorgabe des Autors: maximal 8.000 Wörter Fließtext; ohne die vier Mermaid-Quelltexte, die im Word-Dokument zu Abbildungen werden, liegt der Text bei rund 7.990 Wörtern). Struktur unverändert: alle Kapitel 1–10 mit sämtlichen Unterabschnitten, alle vier Abbildungen, die zentralen Tabellen (Tech Stack Canvas, Teststufen, CI-Jobs, Tabellen 8–9, Plan/Ist) und alle Pflichtpunkte der Prüfungsvorgabe einschließlich der begründeten Negativbefunde (TypeScript, CI/CD, Deployment, Lighthouse, `main`-Rückstand, Prompt-Log-Lücke). Gekürzt wurden Detailtiefe und Zweitbeispiele: verworfene-Alternativen-Tabellen zu je einem Satz verdichtet, Rechenherleitungen auf die Ergebniszahlen reduziert (Dash-Formeln, Auswahl-Hash), Modul-Aufzählungen zu Paketlisten, die E2E-Spec- und Commit-Typ-Tabellen zu Prosa, der Befehlsblock in 9.1 zu einer Zusammenfassung mit Verweis auf `09-quellcode-uebersicht.md`. Schreibstil (Deutsch, unpersönlich, Präsens, Begründung je Behauptung, Querverweise mit Nummer und Titel) beibehalten |
| 2026-08-23 | 1,0 | D-01 | Eigenständiges Kapitel „CI/CD & Git" als Übergabedatei `documentation/kapitel-ci-cd-und-git.md` geschrieben, weil die eingereichte Word-Fassung dieses von der Prüfungsvorgabe geforderte Thema nur verstreut in 7.7, 7.10 und 8.3 führte und nicht als eigenes Kapitel. Positioniert als neues Kapitel 8 hinter „7 Tooling", mit einem Übernahmehinweis im Kopf, der die Verschiebung der bisherigen Kapitel 8–12 auf 9–13 und die Umnummerierung der Tabellen benennt; alle Querverweise im Text sind bereits in der neuen Zählung geschrieben. Inhaltlich vier Abschnitte: Git-Branchmodell und Commit-Konvention, der CI-Ist-Stand, der Vier-Job-Entwurf und das Deployment. Alle Zahlen neu am Repository erhoben statt aus den Kapiteln übernommen: 109 Commits auf `dev`, `main` bei **einem** Commit (der Rückstand ist gegenüber dem in Kap. 7.7 festgehaltenen Stand von 102 auf 108 gewachsen), kein Merge-Commit, keine Tags, Typverteilung als Tabelle. Die fünf Commits mit dem versehentlichen `@ `-Präfix sind in der Tabelle ihrem gemeinten Typ zugeordnet und die Zuordnung ist unter der Tabelle offengelegt, damit die Summe nicht stillschweigend von der Angabe in Kap. 9 abweicht. Abweichung vom Stil der internen Kapitel auf Wunsch des Autors: kürzere Sätze, Fachbegriffe (Pipeline, Job, Merge-Commit) je bei ihrer ersten Verwendung in einem Halbsatz erklärt, keine Aufzählung verworfener Alternativen — die Negativbefunde (keine Pipeline, kein Deployment, `main` ohne stabilen Stand) bleiben aber vollständig und mit Begründung erhalten. Offener Posten: das Kapitel bringt rund 1.100 Wörter mit und sprengt damit die 8.000-Wort-Grenze der Abgabefassung; wo gegengekürzt wird, ist noch zu entscheiden |
| 2026-08-23 | 0,8 | D-01 | Übergabedatei `documentation/code-ausschnitte-kapitel-4-und-5.md` geschrieben: 16 Code-Ausschnitte für den Anhang 11.2 der Word-Fassung plus die 15 Textstellen in Kapitel 4 und 5, an denen sie verlinkt werden. Vorgehen umgekehrt zum Naheliegenden — nicht „welcher Code sieht vorzeigbar aus", sondern die Kapitel 4 und 5 Satz für Satz nach Aussagen durchgegangen, die ohne Beleg nur Behauptung sind, und erst danach die Belegstelle im Repository gesucht. Jede Aussage im Code verifiziert statt aus den Kapiteln übernommen: die dreizehn `Vec2`-Operationen sind dreizehn `pub fn`, die vier privaten Module in `lib.rs` sind vier, die Strides 7/3/5 stehen so in `frame_buffers.rs`. Jeder Ausschnitt trägt seinen Dateipfad, ist gekürzt und die Kürzung mit `// ...` markiert; nichts ist umformuliert. Die Ankersätze aus Teil A sind aus der eingereichten PDF wörtlich zitiert, damit die Einfügung in Word ohne Suchen findbar ist. Zwei Verweise bewusst ohne eigenen Ausschnitt gelöst (5.3 auf Ausschnitt 10 und 15), weil dieselbe Funktion die Aussage schon belegt und der Anhang nicht doppelt wachsen muss. Nebenbefund an den Prüfer weitergegeben statt stillschweigend übergangen: Anhang 11.1–11.3 der eingereichten Fassung enthält drei Fremdinhalte (Fiori-Launchpad, DOM einer Reporting-Kachel, Literaturzeile zu SAP-Regressionstests), die aus einer anderen Arbeit stammen; die Nummerierung der neuen Ausschnitte ist für beide Fälle beschrieben |
| 2026-08-24 | 0,8 | D-01 | Übergabedatei `documentation/vorlage-abgleich-nachtraege.md` geschrieben: 31 kleine Nachträge für die Word-Fassung, jeder mit Ankersatz, fertigem Einfügetext und der Stelle der neuen Vorlage, die er bedient. Grundlage ist der Abgleich der eingereichten PDF gegen die zweite Vorlagenfassung des Prüfers, die gegenüber der Katalogfolie vor allem drei Themen neu verlangt: Versionen und Lockfiles als Nachweis des Tech Stack, Speicher- und Fehlerverhalten an der Sprachgrenze, und einen deutlich ausgebauten KI-Abschnitt (Rollenverteilung, Kontextschichten, eine belegte Fehlannahme, Grenzen). Alle Angaben am Repository geprüft statt aus den Kapiteln übernommen — Versionen aus `package.json` und `Cargo.toml`, Artefaktgrößen aus `frontend/dist/assets/` (46/63/5/14 KB, zusammen rund 129 KB), zehn Frontend-Werkzeuge gegen zwei direkte Rust-Abhängigkeiten, kein `unsafe` in `engine/src`, kein `rust-toolchain`, kein `[profile.release]`, kein Panic-Hook. Vier Nachträge sind dabei Negativbefunde und ausdrücklich als solche formuliert: der nicht abgefangene Browser ohne WebAssembly-Unterstützung, die fehlende Messreihe hinter der Overhead-Aussage aus 1.4/5.2, die nicht festgeschriebene Rust-Toolchain und die fehlende Barrierefreiheitsprüfung. Sechs Vorlagenpunkte bewusst nicht nachgetragen und im Abschnitt „Bewusst nicht nachgetragen" begründet, weil sie neue Tabellen, eine neue Abbildung oder eine Aussage über nicht Vorhandenes wären |
| 2026-08-25 | 0,2 | T-08 | Zusammenführung der konkurrierenden Änderungen in der Prompt-Historie und im Projekt-Journal; beide Eintragsgruppen und das JSON-Format erhalten |
