...
export class PlayerController {
  ...
  update(controls, deltaSeconds, bounds) {
    ...
    if (controls.dash === true && isMoving) {
      ...
      // Holding a direction only ever *adds* to the velocity. Nothing brakes the part that is
      // going somewhere else, so the old momentum has to be spent through this same force before
      // any of it counts towards the new heading — that is the weight the character carries into
      // a turn, and it is deliberate rather than an oversight.
